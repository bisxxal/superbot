...
function SourcesPage() {
    ...
    const createCollections = useMutation({
        mutationFn: async ({ textData, type, collectionName }: { textData: string, type: 'web' | 'text' | 'yt'; collectionName: string }) => {
            return await generateEmbeddings(textData, type, collectionName , 'notebook');
        },
        onSuccess: (data) => {
            if (data) {
                toastSuccess('collection added successfully!');
                client.invalidateQueries({ queryKey: ['notebook_models'] });
            } else {
    ...
    return (
        <div className=" w-full px-10 h-full tex-white overflow-y-scroll pb-20">
            <h2 className=" text-xl mt-5 font-semibold">Upload Context</h2>
            <p className=" text-gray-100 text-sm my-3">Upload documents or add text to train your AI chatbot. The more context you provide, the better your chatbot will perform.</p>
            {
